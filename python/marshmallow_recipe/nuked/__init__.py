...
class _DataclassInfo:
    ...
    effective_naming_case: Callable[[str], str] | None
...
def try_get_dataclass_info(cls: Any, naming_case: Callable[[str], str] | None) -> _DataclassInfo | None:
...
class _BuildContext:
    ...
    def build_root_type(self, cls: Any, naming_case: Callable[[str], str] | None, visited: set[type]) -> Any:
        origin = get_origin(cls)
        args = get_args(cls)
        ...
        if origin is None:
            if dataclasses.is_dataclass(cls):
                ...
                return self.__build_dataclass_type(cls, naming_case, info, visited)
    ...
    def __build_dataclass_core(
        self, cls: Any, naming_case: Callable[[str], str] | None, info: _DataclassInfo, visited: set[type]
    ...
    def __build_dataclass_type(
        ...
        field_handles, field_data_keys = self.__build_dataclass_core(cls, naming_case, info, visited)
        ...
        dc = self.__builder.dataclass(
            info.cls, field_handles, can_use_direct_slots=info.can_use_direct_slots, has_post_init=info.has_post_init
        )
        return self.__builder.type_dataclass(dc)

    def __build_nested_dataclass(self, cls: Any, naming_case: Callable[[str], str] | None, visited: set[type]) -> Any:
        info = _analyze_dataclass(cls, naming_case)
        field_handles, _ = self.__build_dataclass_core(cls, naming_case, info, visited)
    ...
    def __build_enum_field(self, name: str, field_type: type[enum.Enum], optional: bool, kwargs: dict[str, Any]) -> Any:
        ...
        enum_members_repr = "[" + ", ".join(f"{field_type.__name__}.{m.name}" for m in field_type) + "]"

        if issubclass(field_type, str):
            enum_values = [(m.value, m) for m in field_type]
            return self.__builder.str_enum_field(
    ...
    def __build_item_field(self, field_type: Any, naming_case: Callable[[str], str] | None, visited: set[type]) -> Any:
        field_handle, _ = self.__build_field(
            None, "", field_type, None, naming_case, False, nested_naming_case=naming_case, visited=visited
        )
        return field_handle

    def __build_value_field(self, field_type: Any, naming_case: Callable[[str], str] | None, visited: set[type]) -> Any:
        field_handle, _ = self.__build_field(
...
def dump[T](
    ...
    *,
    ...
    info = try_get_dataclass_info(cls, naming_case)
