...
├── load.rs          # Dict → dataclass (main entry)
├── dump.rs          # Dataclass → dict (main entry)
├── load_bytes.rs    # JSON bytes → dataclass
├── dump_bytes.rs    # Dataclass → JSON bytes
├── loader.rs        # Load orchestration
├── dumper.rs        # Dump orchestration
...
└── fields/          # Individual field type handlers
    ├── mod.rs       # Module exports
    ├── any.rs       # Any type pass-through
    ├── bool_type.rs # Boolean handling
    ├── collection.rs# List, Set, FrozenSet, Tuple
    ├── date.rs      # date handling
    ├── datetime.rs  # datetime with timezone
    ├── decimal.rs   # Decimal with Rust-side rounding
    ├── dict.rs      # Dict key/value handling
    ├── float.rs     # Float handling
    ├── helpers.rs   # Shared field utilities
    ├── int.rs       # Integer handling
    ├── int_enum.rs  # IntEnum with direct cast
    ├── nested.rs    # Nested dataclass recursion
    ├── str_enum.rs  # StrEnum with direct cast
    ├── str_type.rs  # String handling
    ├── time.rs      # time handling
    ├── union.rs     # Union type try-match
    └── uuid.rs      # UUID optimized creation
```
