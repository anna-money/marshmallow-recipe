...
* **Breaking:** `mr.nuked.load` no longer accepts `null` as a `dict` value when the value type is not optional; it reports `{"value": ["Field may not be null."]}`, matching `mr`. Previously the Rust backend passed `null` straight through, so `dict[str, int]` could load a `None` value that the annotation forbids.
* `Any` accepts `null` in `mr.nuked` wherever `mr` does — a required `v: Any` field and a `dict[str, Any]` value both load `null` as `None` instead of raising `Field may not be null.`
