...
* Both backends report `dict` entry errors the way `marshmallow.fields.Dict` does, with an independent `"key"` and `"value"` slot per entry, so an entry whose key *and* value are both invalid yields `{"not_int": {"key": [...], "value": [...]}}` rather than only one of the two.
...
* **Breaking:** `mr.nuked.load` no longer accepts `null` as a `dict` value or as a `list`/`tuple`/`set`/`frozenset` item when the element type is not optional; it reports `Field may not be null.`, matching `mr`. Previously the Rust backend passed `null` straight through, so `dict[str, int]` or `list[int]` could load a `None` that the annotation forbids. Optional element types — `dict[str, int | None]`, `list[int | None]` — are unaffected.
